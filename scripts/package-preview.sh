...
if [[ "$ARCH" != *"arm64"* ]]; then
  echo "The public Preview package must contain the verified Apple Silicon executable." >&2
  exit 1
fi

PACKAGE_WORK="$(mktemp -d)"
trap 'rm -rf "$PACKAGE_WORK"' EXIT
STAGE="$PACKAGE_WORK/KE Pen Preview"
DMG="$ROOT/dist/KE-Pen-${VERSION}-arm64-preview.dmg"
CHECKSUM="$DMG.sha256"
...
rm -f "$DMG" "$CHECKSUM"
hdiutil create -volname "KE Pen ${VERSION} Preview" -srcfolder "$STAGE" -ov -format UDZO "$DMG" >/dev/null
...
echo "Built $DMG"
echo "SHA-256 $HASH"
echo "Preview boundary: ad-hoc signed, not Developer ID signed or notarized."
