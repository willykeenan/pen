...
BASE="$ICON_WORK/pen-icon.png"
sips -s format png "$SOURCE" --out "$BASE" >/dev/null
if [[ ! -f "$BASE" ]]; then
  echo "sips did not render the icon source." >&2
  exit 1
fi
...
# Menu-bar template glyph: a dedicated monochrome nib at 18pt + @2x. The
# Template suffix lets macOS recolor it for light and dark menu bars.
TRAY_SOURCE="$ROOT/assets/tray-icon.svg"
TRAY_BASE="$ICON_WORK/tray-icon.png"
sips -s format png "$TRAY_SOURCE" --out "$TRAY_BASE" >/dev/null
if [[ ! -f "$TRAY_BASE" ]]; then
  echo "sips did not render the tray icon source." >&2
  exit 1
fi
...
node "$ROOT/scripts/verify-icon-assets.mjs"
