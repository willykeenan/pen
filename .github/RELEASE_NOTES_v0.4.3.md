...
KE Pen 0.4.3 makes the first-use path much shorter while keeping the same local,
...
Use `KE-Pen-0.4.3-SHA256SUMS.txt` to verify the exact files.
...
builds before the release is published. Version 0.4.3 launches the asar-less
embedded server through the app's packaged Node runtime, avoiding any hidden GUI
or display dependency in the AI-host connection.
