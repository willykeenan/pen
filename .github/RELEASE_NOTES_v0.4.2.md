...
KE Pen 0.4.2 makes the first-use path much shorter while keeping the same local,
...
Use `KE-Pen-0.4.2-SHA256SUMS.txt` to verify the exact files.
...
builds before the release is published. Version 0.4.2 uses an explicit local
launch marker so the same copied setup enters MCP mode reliably across all three
Electron launchers.
